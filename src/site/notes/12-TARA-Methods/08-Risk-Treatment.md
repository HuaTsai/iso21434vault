...
ISO 21434 §15.9.2 [RQ-15-20] 採用與 **ISO 31000** 一致的四個風險處置策略：

| 選項          | 英文 (Clause 15.9 用詞) | 中文 | 適用                             |
| ------------- | ----------------------- | ---- | -------------------------------- |
| **A**voiding  | Avoid                   | 避免 | 移除功能/設計變更                |
| **R**educing  | Reduce                  | 降低 | 加入資安控制（最常見）           |
| **S**haring   | Share                   | 分擔 | 給供應商（責任分擔）、保險、契約 |
| **R**etaining | Retain                  | 保留 | 接受作為殘餘風險（需文件化）     |

> [!note]
>
> - 業界俗稱「**4T**」（Avoid/Reduce/Share/Retain 的助記），但 **ISO 21434 與 ISO 31000 本身不用 4T 這個縮寫**
> - 舊版 ISO 31000:2009 用 "Transfer"，**2018 版已改為 "Sharing"** — 兩者意涵相同（將部分風險分攤給其他方）
> - 「Share/Transfer」並列只是因為業界仍混用，標準新詞為 **Sharing**
...
> - **不強制**使用
> - 業界部分組織採用
> - 在 ISO 21434 中扮演**類似** ASIL（ISO 26262）的角色（指定活動嚴謹度），但兩者推導方式與框架**完全獨立**
